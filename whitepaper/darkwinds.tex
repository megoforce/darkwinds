...
\caption{First draft of Card Model \#19 (Jordmundgander)} \label{fig:A}
\end{figure}


The card smart contract performs all the functions according to the ERC721 \cite{nft} standard. All cards are generated with a payable function called \textbf{getBoosterPack} and the amount of cards returned to the owner is determined by the price of cards, which is set by the contract owner using the \textbf{setCardPrice} function
\subsection{getCard}
...
Cards are generated with a KECCAK-256 operation on the last block timestamp and a modulo operation of 50. Every 5 cards we add another 50 modulo on the contract nonce in attempt to make a rare card. If both modulo operations are larger than 25, a rare card appears. Cards with Model ID higher than 50 are not often generated as shown in Figure A.
